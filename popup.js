...
const optionsConfig = [
    {
        ...
        containerId: "webhook_container",     // El ID del contenedor a ocultar/mostrar
    ...
    },
    {
        ...
        containerId: "WindowUrl_container",     // El ID del contenedor a ocultar/mostrar
...
function setupOptionListeners(config) {
    ...
    if (!optionCheckbox || !container || !inputField) {
        console.warn(`Advertencia: Faltan elementos para la configuración con ID "${config.optionId}".`);
        return;
    }
    ...
    optionCheckbox.checked = window.localStorage.getItem(config.optionStorageKey) === "true";
    ...
    inputField.value = window.localStorage.getItem(config.storageKey) || "";
...
function resetOptions(arrayStrings) {
    ...
    arrayStrings.forEach((key) => {
        window.localStorage.removeItem(key);
        chrome.storage.local.remove(key);
    });
    const resetElements = arrayStrings.map((key) => {
