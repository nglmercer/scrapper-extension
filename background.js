...
async function loadSettings() {
  try {
    ...
    WindowUrl =
      result.WindowUrl || "https://nglmercer.github.io/multistreamASTRO/chat";
...
chrome.storage.onChanged.addListener((changes, namespace) => {
  if (namespace === "local") {
    ...
    if (changes.WebhookOption)
      WebhookOption = changes.WebhookOption.newValue || false;
...
function getOrCreateChatTab() {
  ...
  if (tabCreationPromise) {
    console.log(
      "Una operación de creación de pestaña ya está en curso. Esperando su resultado...",
    );
  ...
  tabCreationPromise = new Promise(async (resolve, reject) => {
    try {
      ...
      if (tabs.length > 0) {
        ...
        console.log(
          "Pestaña de chat no encontrada, creando una nueva en segundo plano.",
        );
...
chrome.tabs.onRemoved.addListener((tabId, removeInfo) => {
  if (tabId === chatTabId) {
    console.log(
      "El usuario ha cerrado la pestaña de chat. Reiniciando estado.",
    );
